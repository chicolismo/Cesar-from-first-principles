...
struct Cpu {
  static const std::size_t ADDRESSABLE_MEMORY_SIZE =
      MEM_SIZE + 8 * sizeof(std::int16_t);
  ...
  [[nodiscard]] inline std::int16_t bytes_to_word(
      const std::int8_t msb, const std::int8_t lsb) const {
    return (msb << 8) | lsb;
  }
  ...
  [[nodiscard]] std::int8_t *get_memory();

  bool read_memory_from_binary_file(const std::string &filename);

  void execute_next_instruction();

  [[nodiscard]] std::uint16_t get_absolute_address(AddressMode mode, int register_number);

  [[nodiscard]] std::int16_t get_value_from_absolute_address(std::size_t address);
...
struct Alu {
  ...
  [[nodiscard]] inline bool is_negative(std::int16_t value) const {
    return value < 0;
  };


  [[nodiscard]] inline bool is_zero(std::int16_t value) const {
    return value == 0;
  }


  [[nodiscard]] inline bool is_overflow(
      std::int16_t a, std::int16_t b, std::int16_t c) const {
    return ((a > 0) && (b > 0) && (c < 0)) || ((a < 0) && (b < 0) && (c > 0));
  }


  [[nodiscard]] inline bool is_carry(
      std::int16_t a, std::int16_t b, CarryOperation operation) const {
    const auto ua = static_cast<std::uint16_t>(a);
    const auto ub = static_cast<std::uint16_t>(b);
    if (operation == Plus) {
      const uint32_t result = ua + ub;
  ...
  std::int16_t one_operand_instruction(
      Instruction instruction, std::int16_t value);

  std::int16_t two_operand_instruction(
      Instruction instruction, std::int16_t src, std::int16_t dst);
