...
std::int16_t Cpu::get_word(std::int16_t address) const {
    ...
    if (IS_DISPLAY_ADDRESS(unsigned_address)) {
        ...
        const auto lsb = static_cast<std::uint8_t>(memory[unsigned_address + 1]);
...
void Cpu::execute_next_instruction() {
    ...
    if (instruction == CONDITIONAL_BRANCH) {
        ...
        const auto next_unsigned_byte = static_cast<std::uint8_t>(get_byte(PC++));
        ...
        case JSR: {
            const auto next_unsigned_byte = static_cast<std::uint8_t>(get_byte(PC++));
            const std::int16_t word = bytes_to_word(unsigned_byte, next_unsigned_byte);
        ...
        default: {
            if (IS_TWO_OPERAND_INSTRUCTION(instruction)) {
                const auto next_unsigned_byte = static_cast<std::uint8_t>(get_byte(PC++));
                const auto uword = static_cast<std::uint16_t>((unsigned_byte << 8u) | next_unsigned_byte);
                ...
                const std::int16_t src = get_value_from_absolute_address(src_address);
                const std::int16_t dst = get_value_from_absolute_address(dst_address);
                const std::int16_t value = alu->two_operand_instruction(instruction, src, dst);
...
void Cpu::set_value_to_absolute_address(const std::int16_t value, const std::size_t address) {
...
std::uint16_t Cpu::get_absolute_address(const AddressMode mode, const int register_number) {
    ...
    case AddressMode::INDEXED: {
        ...
        PC += 2;
        address = static_cast<std::uint16_t>(registers[register_number] + next_word);
    }
        break;

    case AddressMode::REGISTER_INDIRECT: {
        const auto operand_address = static_cast<std::uint16_t>(registers[register_number]);
    ...
    case AddressMode::INDEXED_INDIRECT: {
        const std::int16_t next_word = get_word(PC);
        PC += 2;
        const auto operand_address = static_cast<std::uint16_t>(next_word + registers[register_number]);
        address = get_word(operand_address);
    }
        break;
    }
